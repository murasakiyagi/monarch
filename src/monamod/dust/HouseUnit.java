...
public class HouseUnit extends ImageView {
	...
	public HouseUnit(/*Group p*/Pane p, String name, String[][] paths, int row, int col, int hitP, int team, PeopleUnit cb) {
		//this.p = p;
		this.name = name;
		this.team = team;
		this.paths = paths;
	...
	void masuidou(double hakoW, double ajustW, double ajustH) {//hakoWは一マスの横幅
		this.setX(fl.kitenX - ( row * hakoW/2 ) + ( col * hakoW/2 ) + ajustW);
		this.setY(fl.kitenY + ( col * hakoW/4 ) + ( row * hakoW/4 ) + ajustH);
		hpTx.setX( this.getX() );
		hpTx.setY( this.getY() );
	}

	//kaokulistに入れる。唯一
	void inList(ArrayList<HouseUnit> arr, HouseUnit newKb) {//ダブり防止
	...
	//=============================================
	//ギフター
	HouseUnit copy(int r, int c) { //throws Exception使用する場合引数かっこの後に置き、実装側でtry-catchする
		HouseUnit cpy = new HouseUnit(p, name, paths, r, c, 100, team, cb);
		return cpy;
	}
	...
	void hagukumi() {//キャラの生成
		...
		if(hitP >= 150) {
				//fl.panelPrint();
			PeopleUnit clcb = cb.copy(row, col);
			clcb.padd();
			cm.addCm(clcb);
